...
public class VehicleManagement {
    ...
    public boolean insertVehicle(Vehicle v) {
        ...
        try (Connection con = DBConnectionManager.getConnection();
            ...
            System.out.println("⚠ Duplicate vehicleId or license plate: " + icv.getMessage());
            return false;
        } catch (Exception e) {
    ...
    public boolean insertVehicleBatch(List<Vehicle> vehicles) {
        ...
        try (Connection con = DBConnectionManager.getConnection();
            ...
            for (int i : res) if (i >= 0) success++;
            System.out.println("✅ Batch insert: " + success + " / " + vehicles.size() + " inserted.");
            return true;
        } catch (Exception e) {
    ...
    public boolean isVehicleExists(String vehicleId) {
        try (Connection con = DBConnectionManager.getConnection();
             PreparedStatement ps = con.prepareStatement("SELECT COUNT(*) FROM vehicle WHERE vehicleId = ?")) {
    ...
    public List<Vehicle> getVehiclesByType(String type) {
        ...
        String sql = "SELECT * FROM vehicle WHERE UPPER(vehicleType)=?";
        try (Connection con = DBConnectionManager.getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {
    ...
    public List<Vehicle> getAvailableVehicles() {
        List<Vehicle> list = new ArrayList<>();
        String sql = "SELECT * FROM vehicle WHERE status = 'AVAILABLE' ORDER BY vehicleId";
        try (Connection con = DBConnectionManager.getConnection();
             PreparedStatement ps = con.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {

            while (rs.next()) {
                list.add(mapResultToVehicle(rs));
            }
        } catch (Exception e) {
            System.out.println("⚠ GetAvailableVehicles error: " + e.getMessage());
        }
        return list;
    }
